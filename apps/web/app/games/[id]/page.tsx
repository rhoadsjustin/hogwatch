...
export default async function GamePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="shell detailPage">
    ...
    </> : <>{game.prediction && <PredictionCard prediction={game.prediction} />}<section className="pregameCard"><span className="overline">{isFinal ? 'GRADE PENDING' : 'PREGAME DASHBOARD'}</span><h2>{isFinal ? 'The result is final. The grade is not.' : 'The evidence starts before kickoff.'}</h2><p>{isFinal ? 'HogWatch will add its scorecard after the underlying advanced metrics have been independently verified.' : 'The call above uses the current camp, form, comparison, and location inputs. Once the game is final, the same scorecard will show what changed from the season baseline.'}</p><div><b>Watch after kickoff</b><span>Pressure allowed · Early-down success · Four-man pressure · Explosives</span></div></section></>}
